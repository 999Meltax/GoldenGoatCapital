...
async function addCategory(e) {
    ...
    try {
        const res = await fetch('/users/categories/add', {
            method: 'POST',
...
async function confirmDeleteCategory(name) {
    try {
        const res = await fetch(`/users/categories/delete/${encodeURIComponent(name)}`, { method: 'DELETE' });
        if (!res.ok) throw new Error((await res.json()).message);
...
async function confirmEditCategory(oldName) {
    ...
    if (!newName) return;
    try {
        const res = await fetch('/users/categories/update', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
function showEditTransactionModal(transaction) {
    ...
    fetch('/users/accounts').then(r => r.json()).then(accounts => {
...
async function loadRecurring() {
    try {
        const res = await fetch('/users/recurring');
        if (!res.ok) return;
...
async function quickBook(id, btn) {
    ...
    try {
        const res = await fetch('/users/recurring/' + id + '/book', { method: 'POST' });
        const data = await res.json();
...
async function saveRecurring() {
    ...
    try {
        let res;
        if (id) {
            res = await fetch('/users/recurring/' + id, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
        } else {
            res = await fetch('/users/recurring/add', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
...
async function deleteRecurring() {
    ...
    try {
        const res = await fetch('/users/recurring/' + id, { method: 'DELETE' });
        if (!res.ok) throw new Error();
        await loadRecurring();
        renderRecurringListe();
        switchRecurTab('liste');
